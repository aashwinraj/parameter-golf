...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Older PyTorch builds may not support enable_gqa in SDP.
...
class BatchedLinearLoRA(nn.Module):
    """Per-example LoRA for one linear layer."""
...
class BatchedTTTLoRA(nn.Module):
    """All per-example LoRAs used during TTT."""
...
def _accumulate_bpb(
    ...
    lbl = per_token_losses[batch_i, chunk_offset:chunk_offset + chunk_len].to(torch.float64)
    ...
    tok_bytes += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(dtype=torch.float64)
    loss_sum += lbl.sum()
        ...
        for ci in range(max_nc):
            ...
            if needs_train:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    per_token_losses = base_model(x, y, lora=cur_lora)
            else:
            ...
            if needs_train:
                ...
                per_doc = per_token_losses[:, chunk_offset : chunk_offset + chunk_size].mean(dim=-1)
                cur_opt.zero_grad()
